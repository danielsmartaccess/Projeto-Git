...
class Minesweeper:
    def __init__(self, master):
        ...
        # Criar timer label
        self.timer_label = tk.Label(self.control_frame, text="Tempo: 0", font=('Arial', 12))
        self.timer_label.pack(side=tk.LEFT, padx=10)
        
        # Criar botão de reiniciar
        self.restart_button = tk.Button(self.control_frame, text="Novo Jogo", command=self.restart_game)
    ...
    def reveal_cell(self, x, y):
        ...
        button.configure(state='disabled', relief='sunken')
        
        if mines > 0:
            button.configure(text=str(mines))
        else:
